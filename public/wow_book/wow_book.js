...
function wowBook(elem, opts) {
	...
	if (this.opts.keyboardNavigation) {
		...
		$(document).on("keydown.wowbook", function(e){
			...
			if (book.opts.lightbox && !book.lightboxOn) return;
...
wowBook.prototype = {
	...
	responsive : function(){
		...
		$(window).on("resize.wowbook", function(){
			book.responsiveResizeHandler();
		})
	}, // responsive

	responsiveResizeHandler : function(){
		if ( $.wowBook.utils.isIOS ) {
			var maxWaitTime = 800; // in ms
			var c = (this.opts.container==window && this.container) || (this.lightboxOn && this._lightboxElem);
			var win = $(window);
			if ( c && (c.outerHeight()!=win.height() || c.outerWidth()!=win.width()) ) {
				if (!this.safariFixTotalDelayedTime) this.safariFixTotalDelayedTime=0;
				if ( (this.safariFixTotalDelayedTime||0) < maxWaitTime) {
					var book = this;
					setTimeout(function(){
						book.safariFixTotalDelayedTime = book.safariFixTotalDelayedTime+100;
						book.responsiveResizeHandler();
					},100);
					return;
				}
	...
	*/
	...
	curl : function(page, bottom){
		...
		var onleft = this.pageIsOnTheLeft(page.pageIndex),
		    x0 = onleft  ? 0 : page.pageWidth || this.pageWidth,
		    x1 = x0 + this.opts.curlSize*(onleft ? 1 : -1),
		    y0 = !bottom ? 2 : (page.pageHeight || this.pageHeight)-1,
	...
	initThumbnails: function(){
		...
		this.thumbnailsClipper.on("scroll", function(){
			if ( book._scrollDebounceTimeout ) clearTimeout( book._scrollDebounceTimeout );
			book._scrollDebounceTimeout = setTimeout( function(){
				book.updateVisibleThumbnails();
			}, book.opts.thumbnailsScrollDebounceWait );
		});

	}, // thumbnails

	destroyThumbnails : function(){
		if (this._scrollDebounceTimeout) clearTimeout( this._scrollDebounceTimeout );
	...
	_setThumbnailsContainerWidth : function(){
		var container = this.thumbnailsContainer;
		if (this.opts.thumbnailsContainerHeight) container.height( this.opts.thumbnailsContainerHeight )
		if (this.opts.thumbnailsContainerWidth) {
			container.width( this.opts.thumbnailsContainerWidth )
		} else {
			if ( this.opts.thumbnailsVertical ) {
				var config = this.thumbnailConfig();
				container.width( config.width*2 );
			}
		}
	}, // _setThumbnailsListWidth
	...
	*/
		...
		this.pdfDoc.getPage( pdfIndex ).then(function(page){
			...
			var pageRenderedCallback = function(){
				...
				if ( type!="thumb" ) {
					...
					var viewportBasic = page.getViewport( 1*book.pdfCSS_UNITS*book.opts.pdfScale );
					var factorToFitWidthInsidePdfWidth = book.pdfWidth/viewportBasic.width;
					var viewportForAnnotations = page.getViewport( factorToFitWidthInsidePdfWidth*book.pdfCSS_UNITS*book.opts.pdfScale );
					...
					if ( !bookPage.find('.wowbook-pdf-annotations').length ) {
						...
						book.pdfAnnotations( page, viewportForAnnotations, annotationsDiv );
					}

					if ( book.opts.pdfTextSelectable && !bookPage.textLayer && type!="thumb" ) {
						book.pdfTextLayer( page, pageIndex, viewportForAnnotations );
					}
				}
	...
	showLightbox: function(){
		...
		if ( $.wowBook.utils.isIOS ) $("html").addClass("wowbook-lightbox-ios");
		...
		if (this.mobileToolbar) this.updateMobileToolbar();
		if (this.opts.onShowLightbox) this.opts.onShowLightbox(this, this.pages[this.currentPage], this.currentPage);
	}, // showLightbox
	...
	controllify : function(controls){
		...
		var controlList = ("zoomIn zoomOut zoomReset left lastLeft right lastRight next back first last slideShow flipSound "
		                  +"thumbnails fullscreen toc currentPage download find").split(" "),
		    controlName;
